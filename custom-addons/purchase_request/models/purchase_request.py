...
class PurchaseRequest(models.Model):
    ...
    can_approve = fields.Boolean(compute='_can_approve')
    ...
    def _can_approve(self):
        for rec in self:
            rec.can_approve = True if rec.env.user.id == rec.assigned_to.id else False
    ...
    def to_approve_allowed_check(self):
        for rec in self:
            if not rec.assigned_to:
                raise UserError(_('Please fill Approver!'))
