            ...
            if line.qty_received_method == 'stock_moves':
                    ...
                    for move_is in pick.move_ids_without_package:
                        if move_is.product_id in filtered:
                            if move_is.pack_id in line.product_id.pack_ids:
                                if move_is.product_uom_qty > 0:
                                    quantity = move_is.pack_id.qty_uom / move_is.product_uom_qty
                                    vals_list.append(quantity)
                                move_list.append(move_is.product_uom_qty)
                                done_list.append(move_is.product_uom_qty)
                ...
                if line.product_id.is_pack == True:
                    ...
                    for move in stock_move:
                        if count == 0:
                            if move.state == 'done' and move.product_uom_qty == move.product_uom_qty:
                                flag = True
                            ...
                            flag = 'confirmed'
                            count = count + 1
                            done_list.append(move.product_uom_qty)
                            for picking in picking_ids:
                                for move_is in picking.move_ids_without_package:
